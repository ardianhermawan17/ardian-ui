...
export async function build(relativePath) {
    ...
    /** @type {PackageJson} */
    let packageJson;
    try { packageJson = JSON.parse(packageJsonContents); } catch {
    ...
    // Nanti dipakai aja kalo sempet bikin helper utilities
    // if (pkg === 'ardian-ui') {
    ...
    const esbuildConfig = {
        ...
        // IMPORTANT: externalize internal workspace packages (avoid bundling them)
        external: ['@ardian-ui/*', 'react', 'react-dom'],
        packages: 'external',
        bundle: true,
        sourcemap: true,
        format: 'cjs',
